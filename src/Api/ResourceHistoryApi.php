...
/**
 * ResourceHistoryApi Class Doc Comment.
 *
 * @category Class
 * @package SyncopePhpClient
 * @author OpenAPI Generator team
...
class ResourceHistoryApi {
   ...
   *
   * @param  string $key resource configuration history key to be deleted (required)
  ...
  protected function delete1Request($key, $xSyncopeDomain) {
    ...
    $resourcePath = '/resourcesHistory/{key}';
    $formParams = [];
    $queryParams = [];
    $headerParams = [];
    $httpBody = '';
   ...
   * @return void
   */
  public function list22($resourceKey, $xSyncopeDomain) {
    $request = $this->list22Request($resourceKey, $xSyncopeDomain);
  ...
  /**
   * Create request for operation 'list22'
   *
   * @param  string $resourceKey resource key (required)
   * @param  string $xSyncopeDomain (required)
   *
   ...
   * @return \GuzzleHttp\Psr7\Request
   */
  protected function list22Request($resourceKey, $xSyncopeDomain) {
      // Verify the required parameter 'resourceKey' is set.
    if ($resourceKey === NULL || (is_array($resourceKey) && count($resourceKey) === 0)) {
      throw new \InvalidArgumentException(
        'Missing the required parameter $resourceKey when calling list22'
      );
    }
      // Verify the required parameter 'xSyncopeDomain' is set.
    if ($xSyncopeDomain === NULL || (is_array($xSyncopeDomain) && count($xSyncopeDomain) === 0)) {
      throw new \InvalidArgumentException(
        'Missing the required parameter $xSyncopeDomain when calling list22'
      );
    }

    $resourcePath = '/resourcesHistory/{resourceKey}';
    $formParams = [];
    $queryParams = [];
    $headerParams = [];
    $httpBody = '';
   ...
   *
   * @param  string $key resource configuration history key to be restored (required)
   * @param  string $xSyncopeDomain resource configuration history key to be deleted (required)
   *
   * @throws \SyncopePhpClient\ApiException on non-2xx response
   * @throws \InvalidArgumentException
   * @return void
   */
  public function restore1($key, $xSyncopeDomain) {
    $request = $this->restore1Request($key, $xSyncopeDomain);
  ...
  /**
   * Create request for operation 'restore1'
   *
   * @param  string $key resource configuration history key to be restored (required)
   * @param  string $xSyncopeDomain resource configuration history key to be deleted (required)
   *
   ...
   * @return \GuzzleHttp\Psr7\Request
   */
  protected function restore1Request($key, $xSyncopeDomain) {
      // Verify the required parameter 'key' is set.
    if ($key === NULL || (is_array($key) && count($key) === 0)) {
      throw new \InvalidArgumentException(
        'Missing the required parameter $key when calling restore1'
      );
    }
      // Verify the required parameter 'xSyncopeDomain' is set.
    if ($xSyncopeDomain === NULL || (is_array($xSyncopeDomain) && count($xSyncopeDomain) === 0)) {
      throw new \InvalidArgumentException(
        'Missing the required parameter $xSyncopeDomain when calling restore1'
      );
    }
