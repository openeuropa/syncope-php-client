...
/**
 * ConnectorHistoryApi Class Doc Comment.
 *
 * @category Class
 * @package OpenEuropa\SyncopePhpClient
 * @author OpenAPI Generator team
...
class ConnectorHistoryApi {
   ...
   *
   * @param  string $key connector configuration history instance key to be deleted (required)
  ...
  protected function delete1Request($key, $xSyncopeDomain) {
    ...
    $resourcePath = '/connectorsHistory/{key}';
    $formParams = [];
    $queryParams = [];
    $headerParams = [];
    $httpBody = '';
   ...
   *
   * @param  string $connectorKey connector instance key (required)
   * @param  string $xSyncopeDomain xSyncopeDomain (required)
   *
   * @throws \OpenEuropa\SyncopePhpClient\ApiException on non-2xx response
   * @throws \InvalidArgumentException
   * @return void
   */
  public function list6($connectorKey, $xSyncopeDomain) {
    $request = $this->list6Request($connectorKey, $xSyncopeDomain);
  ...
  /**
   * Create request for operation 'list6'
   *
   * @param  string $connectorKey connector instance key (required)
   * @param  string $xSyncopeDomain (required)
   *
   ...
   * @return \GuzzleHttp\Psr7\Request
   */
  protected function list6Request($connectorKey, $xSyncopeDomain) {
      // Verify the required parameter 'connectorKey' is set.
    if ($connectorKey === NULL || (is_array($connectorKey) && count($connectorKey) === 0)) {
      throw new \InvalidArgumentException(
        'Missing the required parameter $connectorKey when calling list6'
      );
    }
      // Verify the required parameter 'xSyncopeDomain' is set.
    if ($xSyncopeDomain === NULL || (is_array($xSyncopeDomain) && count($xSyncopeDomain) === 0)) {
      throw new \InvalidArgumentException(
        'Missing the required parameter $xSyncopeDomain when calling list6'
      );
    }

    $resourcePath = '/connectorsHistory/{connectorKey}';
    $formParams = [];
    $queryParams = [];
    $headerParams = [];
    $httpBody = '';
    ...
    if ($connectorKey !== NULL) {
      $resourcePath = str_replace(
        '{' . 'connectorKey' . '}',
        ObjectSerializer::toPathValue($connectorKey),
        $resourcePath
      );
    }
   ...
   *
   * @param  string $key connector configuration history instance key to be restored (required)
   * @param  string $xSyncopeDomain connector configuration history instance key to be deleted (required)
   *
   * @throws \OpenEuropa\SyncopePhpClient\ApiException on non-2xx response
   * @throws \InvalidArgumentException
   * @return void
   */
  public function restore($key, $xSyncopeDomain) {
    $request = $this->restoreRequest($key, $xSyncopeDomain);
   ...
   * @return \GuzzleHttp\Psr7\Request
   */
  protected function restoreRequest($key, $xSyncopeDomain) {
      // Verify the required parameter 'key' is set.
    if ($key === NULL || (is_array($key) && count($key) === 0)) {
      throw new \InvalidArgumentException(
        'Missing the required parameter $key when calling restore'
      );
    }
      // Verify the required parameter 'xSyncopeDomain' is set.
    if ($xSyncopeDomain === NULL || (is_array($xSyncopeDomain) && count($xSyncopeDomain) === 0)) {
      throw new \InvalidArgumentException(
        'Missing the required parameter $xSyncopeDomain when calling restore'
      );
    }
